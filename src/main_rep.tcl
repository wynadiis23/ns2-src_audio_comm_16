...
set file "outputs/out-$opt(routing)-T$opt(try)-B$opt(buffer)-C$opt(codec)-V$opt(voipflows)-R$opt(routing)-N$opt(nnode)-Mmobility_$opt(nnode)-$nowTime.output"
...
#	x y config
if {$opt(nnode) == 25} {
	source osmfiles/ns2config/config_23_$opt(try).tcl
	#puts $opt(x)
	#puts $opt(y)
} elseif {$opt(nnode) == 35} {
	source osmfiles/ns2config/config_33_$opt(try).tcl
} elseif {$opt(nnode) == 45} {
	source osmfiles/ns2config/config_43_$opt(try).tcl
} elseif {$opt(nnode) == 55} {
	source osmfiles/ns2config/config_53_$opt(try).tcl
} elseif {$opt(nnode) == 65} {
	source osmfiles/ns2config/config_63_$opt(try).tcl
} elseif {$opt(nnode) == 75} {
	source osmfiles/ns2config/config_73_$opt(try).tcl
} else {
...
if {$opt(nnode) == 25} {
	set val(mob) "./osmfiles/mobility/25/mobility_23_$opt(try).tcl"
	#set val(cp) "./traffic-25.tcl"
} elseif {$opt(nnode) == 35} {
	set val(mob) "./osmfiles/mobility/35/mobility_33_$opt(try).tcl"
	#set val(cp) "./traffic-35.tcl"
} elseif {$opt(nnode) == 45} {
	set val(mob) "./osmfiles/mobility/45/mobility_43_$opt(try).tcl"
	#set val(cp) "./traffic-45.tcl"
} elseif {$opt(nnode) == 55} {
	set val(mob) "./osmfiles/mobility/55/mobility_53_$opt(try).tcl"
	#set val(cp) "./traffic-55.tcl"
} elseif {$opt(nnode) == 65} {
	set val(mob) "./osmfiles/mobility/65/mobility_63_$opt(try).tcl"
	#set val(cp) "./traffic-65.tcl"
} elseif {$opt(nnode) == 75} {
	set val(mob) "./osmfiles/mobility/75/mobility_73_$opt(try).tcl"
...
$ns_ use-newtrace
set tracefile	[open "trace/voip-$opt(routing)-$opt(codec)-$opt(nnode)-T$opt(try).tr" w]
#replikasi ke
puts [expr $opt(try)] 
...
set tr_lg "voip-$opt(routing)-$opt(codec)-$opt(nnode)-T$opt(try).tr"
...
set namtrace    [open "nam/voip-$opt(routing)-$opt(codec)-$opt(nnode)-T$opt(try).nam" w]
